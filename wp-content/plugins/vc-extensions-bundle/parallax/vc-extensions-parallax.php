          ...
          wpb_map( array(
            ...
            "params" => array(
              ...
              array(
                "type" => "textarea_html",
                "holder" => "div",
                "heading" => __("Content text, divide each block with &lt;div class=&#039;parallax-conent&#039;&gt;&lt;/div&gt;, please try to edit in Text mode:", "vc_parallax_cq"),
                "param_name" => "content",
                ...
                </div>", "vc_parallax_cq"), "description" => __("", "vc_parallax_cq") ),
              // array(
              //   "type" => "textfield",
...
, default is 0.75, stand for 75% of the browser.", "vc_parallax_cq")
        ...
        function cq_vc_parallax_func($atts, $content=null) {
          ...
          $output = '';
          $output .= '<div class="cq-parallaxcontainer '.$extra_class.'" style="width:'.$containerwidth.';" data-speed="'.$speed.'" data-coverratio="'.$coverratio.'" data-holderminheight="'.$holderminheight.'" data-holdermaxheight="'.$holdermaxheight.'" data-extraheight="'.$extraheight.'">';
          $i = -1;
          $imagewidth = null;
          foreach ($imagesarr as $key => $image) {
              ...
              if(wp_get_attachment_image_src(trim($image), 'full')){
                  ...
                  if($textfirst=="on"){
                      ...
                      $output .= '<div class="cq-parallaximage" data-image="'.$return_img_arr[0].'" data-width="'.$return_img_arr[1].'" data-height="'.$return_img_arr[2].'" data-image-mobile="'.aq_resize($return_img_arr[0], $mobilewidth, null, true, true, true).'" data-link="'.$customlinkarr[$i].'" data-target="'.$custom_links_target.'"></div>';
                  }else{
                      $output .= '<div class="cq-parallaximage" data-image="'.$return_img_arr[0].'" data-width="'.$return_img_arr[1].'" data-height="'.$return_img_arr[2].'" data-image-mobile="'.aq_resize($return_img_arr[0], $mobilewidth, null, true, true, true).'" data-link="'.$customlinkarr[$i].'" data-target="'.$custom_links_target.'"></div>';
                      if($contentarr[$i+1]!=""){
                          $output .= '<section class="cq-parallaxsection" style="color:'.$textcolor.';background:'.$textbackground.';padding:'.$padding.';">';
