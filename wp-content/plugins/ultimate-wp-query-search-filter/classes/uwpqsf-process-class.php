	...
	if ( $query->have_posts() ) {
	  $html .= '<h1>'.__('Search Results :', 'UWPQSF' ).'</h1>';
	   while ( $query->have_posts() ) {
			...
			$html .= '<article><div class="entry-header updates">'.get_the_post_thumbnail().'';
			$html .= '<span class="pull-left bill-title-inner"><a href="'.get_permalink().'" rel="bookmark">'.get_the_title().'</a></span>';
      $html .= '<span class="entry-summary pull-left">'.get_the_excerpt().'</span></article>';
			$html .= '</div>';
