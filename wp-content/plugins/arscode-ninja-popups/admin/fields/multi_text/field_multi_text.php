...
class SNP_NHP_Options_multi_text extends SNP_NHP_Options{	
	...
	*/
	function render(){
		...
		if(isset($this->value) && is_array($this->value)){
			foreach($this->value as $k => $value){
				if($value != ''){
					...
					//echo '<a href="javascript:void(0);" class="nhp-opts-multi-text-remove">'.__('Remove', 'nhp-opts').'</a>';
		...
		//echo '<a href="javascript:void(0);" class="nhp-opts-multi-text-add" rel-id="'.$this->field['id'].'-ul" rel-name="'.$this->args['opt_name'].'['.$this->field['id'].'][]">'.__('Add More', 'nhp-opts').'</a>';
	...
	*/
	function enqueue(){
		...
		wp_enqueue_script(
			...
			SNP_NHP_OPTIONS_URL.'fields/multi_text/field_multi_text.js', 
			array('jquery'),
			time(),
			true
		);
