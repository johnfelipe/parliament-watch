...
/*
maxAdmin.prototype.initFB = (function(d, s, id) {
	  var js, fjs = d.getElementsByTagName(s)[0];
	  if (d.getElementById(id)) return;
	  js = d.createElement(s); js.id = id;
	  js.src = "//connect.facebook.net/en_US/sdk.js#xfbml=1&version=v2.4&appId=270434146461442";
...
{
	var $modal = $('#mb-formresponse');
	$('[data-form]').prop('disabled', false);
	
	// reset to default
	$modal.removeClass('success'); 
	$modal.removeClass('error'); 
	$modal.find('.controls').children().hide(); 
	...
	$modal.find('.content').text(json.body); 
	...
	if (! result)
	{
		...
		$modal.find('.controls').children('.ok').show();
		$('.show_response_modal').trigger('click');  
