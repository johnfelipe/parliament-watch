...
//html in tel field fixed
//New Feature: 'Remember Shortcode Settings'
//order by last name improvement
//updated drag&drop code
//filter nav with children support (dropdown)
//tslocation html fixed
//search message bug fixed
//id for entries in grid layout
//social icons with font icons bug fixed
//single page template options
//fixed settings reset after disable
//added custom css option
//fixed pager/filter css bug
...
function tshowcase_get_information($id,$show,$display=array(),$singular=false) {
    ...
    if(($title)) { 
    $info_array['title'] = '<div class="tshowcase-single-title">'.$icontitle.get_the_title($id).'</div>'.$html;
    }
      ...
      if($tspersonalanchor != '') {
          ...
          $info_array['website'] =   "<div class='tshowcase-single-website'>".$iconpersonal."<a href='".$tspersonal."' target='_blank'>".$tspersonalanchor."</a></div>";     
