<?php
	...
	*/
	...
	function formcraft3_verify_license()
	{
		...
		$response = wp_remote_get("http://formcraft-wp.com?type=register_license&key=".$_POST['key']."&site=".rawurlencode(site_url())."&email=".$_POST['email']);
		...
		{
			$response['purchased'] = date(get_option('date_format'), strtotime($response['purchased']));
			$response['expires'] = date(get_option('date_format'), strtotime($response['expires']));
			$response['registered'] = date(get_option('date_format'), strtotime($response['registered']));
	...
	class FormCraft_Plugin_Updater {
		...
		private function getRepoReleaseInfo() {
			...
			$url = "https://api.github.com/repos/{$this->username}/{$this->repo}/releases?access_token=04b8c72ca61a495672c413669a165e23e42fce9a";
		...
		public function setTransitent( $transient ) {
			...
			$date = DateTime::createFromFormat(get_option('date_format'), get_site_option( 'f3_expires' ));
			if ($date instanceof DateTime)
			{
				$expires_time = $date->getTimestamp();
			}
	...
	*/
	...
	function formcraft3_install_plugin()
	{
		...
		if ( $fc_meta['preview_mode']==true ) {
			echo json_encode(array('failed'=>'Can\'t install plugins in preview mode')); die();
		}
	...
	function formcraft3_redirect_to_form_page()
	{
		...
		function formcraft3_shortcode( $atts, $content = "" ) {
			...
			wp_enqueue_script('fc-form-js', plugins_url( 'assets/js/form.js', __FILE__ ), array('jquery', 'jquery-ui-core', 'jquery-ui-datepicker','jquery-ui-slider','jquery-ui-widget','jquery-ui-mouse'), $fc_meta['version']);
			wp_enqueue_script('fc-autosize-js', plugins_url( 'assets/js/autosize.js', __FILE__ ));
			...
			{
				wp_enqueue_script('fc-modal-js', plugins_url( 'assets/js/fc_modal.js', __FILE__ ));
				$powered_by = get_site_option('f3_verified')!='yes' || ( $fc_meta['f3_multi_site_addon'] == false && get_site_option('f3_blog_id')!=get_current_blog_id() ) ? '' : '';
	...
	*/
	...
	function formcraft3_get_files()
	{
		...
		if ( isset($_POST['query']) && $_POST['query']!='' )
		{
			...
			$files = $wpdb->get_results( $wpdb->prepare( "SELECT id,name,mime,size,file_url,created FROM $fc_files_table WHERE (name LIKE %s or mime LIKE %s) ORDER BY id DESC LIMIT $from, $to;", '%' . $wpdb->esc_like($_POST['query']) . '%', '%' . $wpdb->esc_like($_POST['query']) . '%'), ARRAY_A );
		}
		else
		{
			$total = $wpdb->get_var( "SELECT COUNT(*) FROM $fc_files_table" );
			$files = $wpdb->get_results( "SELECT id,name,mime,size,file_url,created FROM $fc_files_table ORDER BY id DESC LIMIT $from, $to", ARRAY_A );
		}
	...
	function formcraft3_get_template()
	{
		...
		{
			echo json_encode(array('html'=>stripcslashes($content->html),'config'=>json_decode($content->meta_builder)));
		}
		die();
	}
	...
	*/
	...
	function formcraft3_test_email()
	{
		...
		foreach ($emails as $key => $email) {
			...
			{
				if ( !isset($config['smtp_sender_username']) || 
					!isset($config['smtp_sender_password']) || 
					!isset($config['smtp_sender_security']) || 
					!isset($config['smtp_sender_port']) )
				{
					$fc_final_response['debug']['failed'] = __('Incomplete Email Configuration','formcraft');
					echo __('Incomplete Email Configuration','formcraft');
					die();
				}
				else
				{
					...
					$mail->SMTPAuth = true;
					$mail->Username = $config['smtp_sender_username'];
					$mail->Password = $config['smtp_sender_password'];
					$mail->SMTPSecure = $config['smtp_sender_security'];
					$mail->Port = $config['smtp_sender_port'];								
				}
			}
	...
	function formcraft3_form_submit()
	{
		...
		foreach ($content as $key => $value) {
			...
			else
			{
				if ( $value['type']=='heading' )
				{
					$temp2[] = "<div style='font-size: 120%; font-weight: bold'>".$value['value']."</div>";
				}
				else
				{
					$value['value'] = $value['type']=='checkbox' || $value['type']=='fileupload' ? str_ireplace("<br>", ", ", $value['value']) : $value['value'];
					$temp2[] = "<div><strong>".$value['label'].":	</strong><span>".$value['value']."</span></div>";
				}
			}
		...
		if ( isset($autoresponder_email) && is_array($autoresponder_email) && count($autoresponder_email)>0 )
		{
			...
			foreach ($autoresponder_email as $email) {
				...
				{
					if ( !isset($meta['config']['notifications']['smtp_sender_username']) || 
						!isset($meta['config']['notifications']['smtp_sender_password']) || 
						!isset($meta['config']['notifications']['smtp_sender_security']) || 
						!isset($meta['config']['notifications']['smtp_sender_port']) )
					{
						$fc_final_response['debug']['failed'] = __('Incomplete Email Configuration','formcraft');
					}
					else
					{
						...
						$mail->SMTPAuth = true;
						$mail->Username = $meta['config']['notifications']['smtp_sender_username'];
						$mail->Password = $meta['config']['notifications']['smtp_sender_password'];
						$mail->SMTPSecure = $meta['config']['notifications']['smtp_sender_security'];
						$mail->Port = $meta['config']['notifications']['smtp_sender_port'];								
					}
				}
		...
		if ( isset($meta['config']) )
		{
			if ( isset($meta['config']['notifications']['recipients']) )
			{
				...
				if ( is_array($emails) && count($emails)>0 )
				{
					...
					else{$fc_final_response['debug']['success'][] = __($sent.' notification email(s) sent','formcraft');}
				}
			}
		}
	...
	*/
	...
	function formcraft3_email_template($content)
	{
		...
		<html xmlns="http://www.w3.org/1999/xhtml" xmlns="http://www.w3.org/1999/xhtml" style="font-family: Helvetica, Arial, sans-serif; font-size: 100%; line-height: 1.6; margin: 0; padding: 0;">
		<head>
			<meta name="viewport" content="width=device-width" />
			<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
		</head>
		<body style="font-family: Helvetica, Arial, sans-serif; font-size: 100%; line-height: 1.6; -webkit-font-smoothing: antialiased; -webkit-text-size-adjust: none; width: 100% !important; height: 100%; margin: 0; padding: 0;">
			<table style="font-family: Helvetica, Arial, sans-serif; font-size: 100%; line-height: 1.6; width: 100%; margin: 0; padding: 0px;">
				<tr style="font-family: Helvetica, Arial, sans-serif; font-size: 100%; line-height: 1.6; margin: 0; padding: 0;"><td style="font-family: Helvetica, Arial, sans-serif; font-size: 100%; line-height: 1.6; margin: 0; padding: 0;"></td>
					<td style="font-family: Helvetica, Arial, sans-serif; font-size: 100%; line-height: 1.6; display: block !important; clear: both !important; margin: 0; padding: 0px;">
						<div style="font-family: Helvetica, Arial, sans-serif; font-size: 100%; line-height: 1.6; display: block; margin: 0; padding: 0;">
							<table style="font-family: Helvetica, Arial, sans-serif; font-size: 100%; line-height: 1.6; width: 100%; margin: 0; padding: 0;">
								<tr style="font-family: Helvetica, Arial, sans-serif; font-size: 100%; line-height: 1.6; margin: 0; padding: 0;">
									<td valign="top" style="font-family: Helvetica, Arial, sans-serif; font-size: 100%; line-height: 1.6; margin: 0; padding: 0;vertical-align:top;">
										'.$content.'
									</td>
								</tr>
							</table>
