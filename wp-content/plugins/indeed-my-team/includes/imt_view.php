...
if(count($the_posts)>0){
    ...
    if (file_exists($theme_file)){
		...
		foreach ($plugins_arr as $name){
			$dir = str_replace('indeed-my-team', $name, IMT_DIR_PATH );
			$style_url = str_replace('indeed-my-team', $name, IMT_DIR_URL);
    ...
    $default_item = $list_item_template; // $list_item_template is available in theme files
  ...
  $final_str .= '</div>';
    ...
    foreach($the_posts as $post){
        ...
        if ( $attr['page_inside']==1 ){
        	...
        	if (isset($attr['tm_custom_href']) && $attr['tm_custom_href']){
        		$tm_custom_href = get_post_meta($post->ID, 'imt_team_href', TRUE);
        		if ($tm_custom_href){
        			if ($tm_custom_href>0){
        				$link = get_permalink ($tm_custom_href);
        			} elseif($tm_custom_href==-1) {
        				//custom link
        				$link = get_post_meta($post->ID, 'imt_team_href-custom', TRUE);
        			}
        		}
        	}
        }
        if ($link){
        	$target_blank = '';
        	if (isset($general_settings_data['imt_target_blank']) && $general_settings_data['imt_target_blank']) $target_blank = 'target="_blank"';
        	$list_item_template = str_replace("#POST_LINK#", ' href="'.$link.'" '.$target_blank , $list_item_template);        	
        } else {
        	$list_item_template = str_replace("#POST_LINK#", '', $list_item_template);
        }    
