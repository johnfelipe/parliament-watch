...
class EasySocialFansCounter {
	...
	function get_facebook_count_with_token($id, $token) {
		...
		try {
			$access_token = $token;
			$data = @$this->parse ( "https://graph.facebook.com/v2.3/$id?fields=likes&&access_token=$access_token" );
			$result = ( int ) $data ['likes'];
		} catch ( Exception $e ) {
			$result = 0;
		}
	...
	function update_youtube_count() {
		...
		if (! empty ( $this->transient ['youtube'] )) {
			...
			try {
				...
				$result = 0;
				$data_type = isset($this->options ['social'] ['youtube'] ['type']) ? $this->options ['social'] ['youtube'] ['type'] : '';
				$api = isset ( $this->options ['social'] ['google'] ['api'] ) ? $this->options ['social'] ['google'] ['api'] : '';
				if ($data_type == "Channel") {
					$response = @$this->parse ( 'https://www.googleapis.com/youtube/v3/channels?part=statistics&id='.$id.'&key=' . $api );
					
				}
				else {
					$response = @$this->parse ( 'https://www.googleapis.com/youtube/v3/channels?part=statistics&forUsername='.$id.'&key=' . $api );
				}	
