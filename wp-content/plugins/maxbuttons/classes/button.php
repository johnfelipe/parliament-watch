...
class maxButton
{
	...
	*/
	...
	function parse_button($mode = 'normal')
	{
		...
		$domObj->load("<a class=\"maxbutton-" . $this->id . " maxbutton-" . $name . " maxbutton\" ></a>"); 
	...
	*/
	...
	{
		global $wpdb;
		$wpdb->query($wpdb->prepare("DELETE FROM " . maxUtils::get_buttons_table_name() . " WHERE id = %d", $id));
	}
	...
	*/
	...
	public function update_cache($css)
	{
		...
		if ($this->id > 0) 
		{
			...
			$where = array('id' => $this->id);
			$where_format = array('%d');
			$wpdb->update(maxUtils::get_buttons_table_name(), $fields, $where, null, $where_format);
			$return = true;
	...
	{  		 			
		...
		$this->data = apply_filters('mb_shortcode_data', $this->data, $atts); 
