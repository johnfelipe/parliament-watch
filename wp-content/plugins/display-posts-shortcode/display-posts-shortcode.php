...
function be_display_posts_shortcode( $atts ) {
    ...
    if ( $include_date ) 
			$date = ' <span class="date"><p class="date-2">  ' . get_the_date( $date_format ) . ' </p> </span>';
		...
		if( $include_author )
			...
			$author = apply_filters( 'display_posts_shortcode_author', ' <span class="date"> ' .$author_avatar.'<p> By ' . get_the_author(). ' </p></span> ' );
		...
		$output = '<' . $inner_wrapper . ' class="' . implode( ' ', $class ) . '"> <div class="pull-left short-image">' . $image .$title . $author .  '</div> <div class=" pull-left short-content">' . $category_display_text . $excerpt . $content.  '</div></' . $inner_wrapper . '>';
